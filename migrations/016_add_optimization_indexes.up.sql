...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_audit_logs_target_type_id 
    ON audit_logs(target_type, target_id);

-- Add deleted_at index for soft delete performance on attachments
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_attachments_deleted_at 
    ON attachments(deleted_at) WHERE deleted_at IS NULL;

-- Add index for attachment shares
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_attachment_shares_attachment_id 
    ON attachment_shares(attachment_id);

CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_attachment_shares_share_url 
    ON attachment_shares(share_url);

CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_attachment_shares_is_active 
...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_user_preferences_updated_at 
    ON user_preferences(updated_at DESC);

-- Add index for user_statuses by status
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_user_statuses_status 
    ON user_statuses(status);

-- Add index for scheduled_messages by scheduled_at
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_scheduled_messages_scheduled_at 
...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_message_reminders_remind_at 
